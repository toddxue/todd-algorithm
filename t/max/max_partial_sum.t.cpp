...
int max_partial_sum_stupid(int n, int* a) {
    int max = a[0];
    int sum;
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j <= i; ++j) {
...
bool random_check(int n, int max, int c) {
    ...
    for (int cc = 0; cc < c; ++cc) {
        ...
        int sum1 = max_partial_sum(n, a);
        int sum2 = max_partial_sum_stupid(n, a);
...
static
int my_max_partial_sum(int c, ...) {
    int* a = new int[c];
    va_list ap;
    va_start(ap, c);
    for (int i = 0; i < c; ++i)
        a[i] = va_arg(ap, int);
    int sum = max_partial_sum(c, a);
    delete[] a;
    return sum;
}
...
int main(int argc, char* argv[]) {
    ...
    // 1 int
    OK(my_max_partial_sum(1, 1) == 1);
    
    // 2 ints
    OK(my_max_partial_sum(2, 1, 1) == 2);
    OK(my_max_partial_sum(2, 1, -1) == 1);
    OK(my_max_partial_sum(2, -1, 1) == 1);

    // 3 ints
    OK(my_max_partial_sum(3, 1, 1, 1) == 3);

    OK(my_max_partial_sum(3, 1, -1, -1) == 1);
    OK(my_max_partial_sum(3, -1, 1, -1) == 1);
    OK(my_max_partial_sum(3, -1, -1, 1) == 1);

    OK(my_max_partial_sum(3, 2, -1, -1) == 2);
    OK(my_max_partial_sum(3, -1, 2, -1) == 2);
    OK(my_max_partial_sum(3, -1, -1, 2) == 2);

    // 4 ints
    OK(my_max_partial_sum(4, 1, 1, -1, -1) == 2);
    OK(my_max_partial_sum(4, 1, -1, 1, -1) == 1);
    OK(my_max_partial_sum(4, 1, -1, -1, 1) == 1);
    ...
    // 5 ints
    OK(my_max_partial_sum(5, 1, 1, -1, -1, 5) == 5);
